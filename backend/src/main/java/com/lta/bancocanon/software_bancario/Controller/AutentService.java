...
public class AutentService {
    ...
    public AutentResponse login(LoginRequest loginRequest) {
        Usuario usuario = usuarioRepository.findByNomUsuario(loginRequest.getNomUsuario()).orElseThrow();

    authenticationManager.authenticate(
        new UsernamePasswordAuthenticationToken(usuario.getId(), loginRequest.getContrasena())
    );
    ...
    String token = jwtService.getToken(claims, usuario);    
    ...
    */
    ...
    Usuario usuario = Usuario.builder()
                ...
                try {
                    ...
                    throw new ResponseStatusException(HttpStatus.CONFLICT, "Nombre de usuario ya existe");
                }

        // Generar token al registrar (auto-login)
        Map<String, Object> claims = new HashMap<>();
        claims.put("username", usuario.getNomUsuario());
        claims.put("email", usuario.getCorreo());
        String token = jwtService.getToken(claims, usuario);
