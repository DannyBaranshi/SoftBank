...
async function importJWK(jwk, alg, options) {
  ...
  switch (jwk.kty) {
    ...
    case "RSA":
      ...
      return jwkToKey(__spreadProps(__spreadValues({}, jwk), { alg, ext }));
    case "AKP": {
      ...
      return jwkToKey(__spreadProps(__spreadValues({}, jwk), { ext }));
    }
    case "EC":
    case "OKP":
      return jwkToKey(__spreadProps(__spreadValues({}, jwk), { alg, ext }));
...
async function flattenedDecrypt(jwe, key, options) {
  ...
  if (resolvedKey) {
    return __spreadProps(__spreadValues({}, result), { key: k });
  }
  return result;
}
...
async function keyToJWK(key) {
  ...
  const _a = await crypto.subtle.exportKey("jwk", key), { ext, key_ops, alg, use } = _a, jwk = __objRest(_a, ["ext", "key_ops", "alg", "use"]);
  if (jwk.kty === "AKP") {
    ;
    jwk.alg = alg;
  }
...
async function encryptKeyManagement(alg, enc, key, providedCek, providedParameters = {}) {
  ...
  switch (alg) {
    ...
    case "PBES2-HS512+A256KW": {
      ...
      _a = await wrap2(alg, key, cek, p2c, p2s), { encryptedKey } = _a, parameters = __objRest(_a, ["encryptedKey"]);
      break;
    }
    case "A128KW":
    case "A192KW":
    ...
    case "A256GCMKW": {
      ...
      _b = await wrap3(alg, key, cek, iv), { encryptedKey } = _b, parameters = __objRest(_b, ["encryptedKey"]);
      break;
    }
    default: {
      throw new JOSENotSupported('Invalid or unsupported "alg" (JWE Algorithm) header value');
...
var FlattenedEncrypt = class {
  ...
  async encrypt(key, options) {
    ...
    const joseHeader = __spreadValues(__spreadValues(__spreadValues({}, this.#protectedHeader), this.#unprotectedHeader), this.#sharedUnprotectedHeader);
      ...
      if (parameters) {
        if (options && unprotected in options) {
          if (!this.#unprotectedHeader) {
            ...
            this.#unprotectedHeader = __spreadValues(__spreadValues({}, this.#unprotectedHeader), parameters);
          }
        } else if (!this.#protectedHeader) {
          this.setProtectedHeader(parameters);
        } else {
          this.#protectedHeader = __spreadValues(__spreadValues({}, this.#protectedHeader), parameters);
        }
      }
    }
    let additionalData;
...
var GeneralEncrypt = class {
  ...
  async encrypt() {
    ...
    if (this.#recipients.length === 1) {
      ...
      const flattened = await new FlattenedEncrypt(this.#plaintext).setAdditionalAuthenticatedData(this.#aad).setProtectedHeader(this.#protectedHeader).setSharedUnprotectedHeader(this.#unprotectedHeader).setUnprotectedHeader(recipient.unprotectedHeader).encrypt(recipient.key, __spreadValues({}, recipient.options));
      const jwe2 = {
    ...
    for (let i = 0; i < this.#recipients.length; i++) {
      ...
      const joseHeader = __spreadValues(__spreadValues(__spreadValues({}, this.#protectedHeader), this.#unprotectedHeader), recipient.unprotectedHeader);
...
var FlattenedSign = class {
  ...
  async sign(key, options) {
    ...
    const joseHeader = __spreadValues(__spreadValues({}, this.#protectedHeader), this.#unprotectedHeader);
...
var GeneralSign = class {
  ...
  async sign() {
    ...
    for (let i = 0; i < this.#signatures.length; i++) {
      ...
      const _a = await flattened.sign(signature.key, signature.options), { payload } = _a, rest = __objRest(_a, ["payload"]);
      if (i === 0) {
        jws.payload = payload;
...
var EncryptJWT = class {
  ...
  async encrypt(key, options) {
    ...
    if (this.#protectedHeader && (this.#replicateIssuerAsHeader || this.#replicateSubjectAsHeader || this.#replicateAudienceAsHeader)) {
      this.#protectedHeader = __spreadProps(__spreadValues({}, this.#protectedHeader), {
...
async function EmbeddedJWK(protectedHeader, token) {
  ...
  const key = await importJWK(__spreadProps(__spreadValues({}, joseHeader.jwk), { ext: true }), joseHeader.alg);
...
async function importWithAlgCache(cache2, jwk, alg) {
  ...
  if (cached[alg] === void 0) {
    const key = await importJWK(__spreadProps(__spreadValues({}, jwk), { ext: true }), alg);
    if (key instanceof Uint8Array || key.type !== "public") {
